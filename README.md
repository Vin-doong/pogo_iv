...
총 13개 탭, **공통 → PvE → PvP** 순으로 정렬: **공통 6 (대시보드 1 + 타입 1 + 일정 4) + PvE 4 + PvP 3**

## 공통 (6개 탭)

### 오늘 할 일 (대시보드)
- **오늘 할 일** — 맨 위에 **일일/주간 루틴 체크리스트**(무료 레이드 패스·필드 리서치·선물/반짝교환·버디 하트·체육관 점령·맥스 파티클·데일리 인센스·모험싱크·리서치 돌파·스포트라이트 등). 체크 상태는 저장되며 매일/매주 자동 초기화. 이어서 진행 중/곧 시작 이벤트 + 주목 레이드(5★·메가·엘리트) + 알·리서치 요약 타임라인. 맨 아래 **🛒 박스 효율 계산기** — 포켓몬샵 박스의 구성품 개수 + 박스 가격을 넣으면 단품 환산 가치·할인율·가치비를 계산해 🔥매우 좋음~👎비효율 판정(단품 기준가 편집 가능, 입력값 유지)
