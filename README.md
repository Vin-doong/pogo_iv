...
총 14개 탭, **PvP → PvE → 공통** 순으로 정렬(시작 탭 = PvP 분석): **PvP 3 + PvE 4 + 공통 7 (대시보드 1 + 검색어 1 + 타입 1 + 일정 4)**
...
- **PvE 다맥** — 다이맥스/거다이맥스 맥스배틀 추천: 최우선 S급 어택커 + 18타입별 추천 어택커(맥스무브 포함) + 탱커(0.5초 평타 기준) + 힐러(HP 기준) + **출시된 거다이맥스 17종 / 다이맥스 주요 로스터**. 맥스배틀은 일반 레이드와 역학이 달라 별도 큐레이션(출처 GO Hub/doctorpokegogo/Dexerto, 표만 갱신하면 됨). CLI: `--maxtier`. *다이맥스/거다이맥스는 별도 종이 아니라 전투 상태 — 종 자체는 좌측 목록에 이미 있고, 게임 내 `다이맥스`/`거다이맥스` 검색으로 보유분 필터*
...
### 검색어
- **검색어** — 포켓몬GO 인게임 검색 문자열 모음(개체값/어필 등급, 세부 IV 공·방·체, 상태·속성, 배틀·레이드·맥스, 레벨/종류, 박사 전송용 조합, 연산자). 각 줄 **[복사]** 버튼으로 클립보드에 담아 게임 검색창에 붙여넣기. CLI: `--searchhelp`
...
python pogo_iv.py --searchhelp               # 인게임 검색어 사전 전체 (복사용)
...
### 2026-07-24
- **PvP 최적 IV를 PvPoke 커뮤니티 기준과 일치 (`pogo_iv.py`)** — 슈퍼/하이퍼리그 '최적 IV'가 PvPoke·PoGoMate 등과 어긋나던 문제 수정 (예: 엠페르트 슈퍼리그 → 기존 `0/12/15`, 수정 후 `0/13/15`)
  - 원인: 반레벨(Lv19.5 등) CPM을 우리는 Niantic 공식값(`0.590043681`)으로, PvPoke는 인접 정수레벨의 **제곱평균** `sqrt((cpm_lo²+cpm_hi²)/2)`(`0.589888`)으로 계산 → CP 1500/2500 캡 경계에서 벌크형(저공격) IV의 반레벨 안착 여부가 1 CP 차이로 갈려 1등이 달라짐 (PvPoke `Pokemon.js` 소스에서 공식 확인)
  - 랭킹 전용 `CPM_PVP` 테이블 신설(정수레벨은 공식값과 동일, 반레벨만 제곱평균) → `rank_all`이 이를 사용
  - **개체값 계산기(CP→IV 역추적)·인게임 CP 표시는 공식 CPM 그대로 유지** — 실제 게임 CP와의 정확도 보존. 즉 PvP 랭킹만 커뮤니티식, 역추적은 게임 정확값

### 2026-07-07
- **봇 IV 도구 분리 + 리그 필터 (`bot.py`)** — 기존 `analyze_pokemon` 단일 도구를 용도별 2개로 분리
  - `league_info(names, leagues)` — 사용자가 IV를 명시하지 **않았을 때** 리그별 최적 IV·메타 랭킹·Lv/CP 조회. 여러 포켓몬·리그 한 번에 처리
  - `analyze_user_iv(name, atk, defense, hp, leagues, best_buddy)` — 사용자가 **IV를 직접 준 경우만** 그 IV의 리그별 순위/CP 분석. `best_buddy=True` 시 Lv51 캡(기본 Lv50, PvPoke 기준)
  - 공용 헬퍼 `_normalize_leagues`(리그명 별칭 정규화) / `_meta_rank_for_sid`(전체 메타 랭킹 위치) 추가
  - 시스템 프롬프트 도구 선택 가이드도 두 도구 기준으로 갱신
  - PvE 신규 도구 6종(`type_attackers`·`investment_priority`·`max_battle_tier`·`search_string`·`fusion_energy`·`daily_routine`)과 병행 등록 (TOOLS 13종)
- **버그 수정: `_clean_args` int 강제변환 무력화** — `from __future__ import annotations` 로 `_TOOL_PARAMS` 어노테이션이 문자열(`'int'`)이 되어 `is int` 검사가 항상 실패, 모델이 IV를 문자열로 보내면 형변환이 안 되던 문제. `inspect.signature(fn, eval_str=True)` 로 실제 타입 평가하도록 수정
