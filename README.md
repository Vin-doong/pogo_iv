...
### 2026-04-26
- **PvE 로켓 탭에 간부/지오반니 분석 도우미 섹션 추가**
  - 기존엔 "PvE 카운터 탭의 좌측 선택 포켓몬을 보스로 모드를 활용하세요" 라는 추상적 안내만 있어 사용 흐름이 모호했음
  - 새 LabelFrame "간부 / 보스 지오반니 분석" 안에 3단계 가이드 + 현재 좌측 선택 포켓몬 라벨 + [선택한 포켓몬을 보스로 분석 →] 버튼 추가
  - 버튼 클릭 시 자동으로 PvE 카운터 탭으로 점프 + use_selected 모드 ON + 새로고침
  - 좌측 리스트 선택 변경 시 라벨 실시간 갱신
